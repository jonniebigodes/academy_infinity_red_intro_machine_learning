...
import {
  generateTensorDataBw,
  generateTensorDataRGB,
  generateOffTensor,
  generateOnTensor,
} from "../utils/helpers"
import useInterval from "../hooks/useInterval"
export default () => {
  const [offTensor] = useState(generateOffTensor())
  const [onTensor] = useState(generateOnTensor())
  ...
  useEffect(() => {
    ...
    console.log(
      `useeffect ghostImageRef.current.width:${ghostImageRef.current.width}`
    )
    ...
    const imageWithNoise = [...Array(20).keys()].map(x => {
      return tf
        .randomUniform([250, 250, 3], 0, 255, "int32")
        .add(imageToTensor)
        .div(512)
    })
